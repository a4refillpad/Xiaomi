...
metadata {
	...
	preferences {
		...
		input "motionreset", "number", title: "Enter the number of seconds of inactivity required before resetting to 'no motion'", description: "Range 1-60 (default is 7)", range: "1..60"
		//Date & Time Config
...
// Parse motion active/inactive report or battery report on reset button press
...
def stopMotion() {
	if (device.currentState('motion')?.value == "active") {
		...
		log.debug "${device.displayName} reset to no motion after ${seconds} seconds of inactivity"
	}
}
