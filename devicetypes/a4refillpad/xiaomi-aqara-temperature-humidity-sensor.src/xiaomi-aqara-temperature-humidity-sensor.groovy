...
metadata {
	...
	tiles(scale: 2) {
        ...
        standardTile("pressure", "device.pressure", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
			state "default", label:'${currentValue} mb', icon:"st.Weather.weather1"
		}
...
def parse(String description) {
	...
	def unit = (name == "temperature") ? getTemperatureScale() : ((name == "humidity") ? "%" : ((name == "pressure")? "mb": null))
...
private String parseReadAttrMessage(String description) {
    ...
    if (cluster == "0403" && attrId == "0000") {
         ...
         if (pressOffset)
         {
           result = ((pressureval/10) as Float) + pressOffset
         }
         else
         {
           result = (pressureval/10 as Float)
         }
    } 
